...
_parser.add_argument(
    "--load_path", type=str, default=None, help="load ckpt path"
)
## optim
_parser.add_argument(
    "--optim",
...
_parser.add_argument(
    ...
    type=float,
    default=1e-4,
    help="learning rate for all types of optim",
)
_parser.add_argument(
...
_parser.add_argument(
    "--step_size", type=int, default=100, help="step size for StepLR"
)
_parser.add_argument(
    "--gamma", type=float, default=0.5, help="gamma for StepLR"
)
...
## GradVac
_parser.add_argument(
    "--GradVac_beta", type=float, default=0.5, help="beta for GradVac"
)
_parser.add_argument(
    "--GradVac_group_type",
    type=int,
...
## GradNorm
_parser.add_argument(
    "--alpha", type=float, default=1.5, help="alpha for GradNorm"
)
## GradDrop
_parser.add_argument(
    "--leak", type=float, default=0.0, help="leak for GradDrop"
)
## CAGrad
_parser.add_argument(
    "--calpha", type=float, default=0.5, help="calpha for CAGrad"
)
_parser.add_argument(
    "--rescale", type=int, default=1, help="rescale for CAGrad"
)
## Nash_MTL
_parser.add_argument(
...
## MoCo
_parser.add_argument(
    "--MoCo_beta", type=float, default=0.5, help="MoCo_beta for MoCo"
)
_parser.add_argument(
    "--MoCo_beta_sigma",
    type=float,
    default=0.5,
    help="MoCo_beta_sigma for MoCo",
)
_parser.add_argument(
    "--MoCo_gamma", type=float, default=0.1, help="gamma for MoCo"
)
_parser.add_argument(
    "--MoCo_gamma_sigma",
    type=float,
    default=0.5,
    help="MoCo_gamma_sigma for MoCo",
)
_parser.add_argument(
    "--MoCo_rho", type=float, default=0, help="MoCo_rho for MoCo"
)
## DB_MTL
...
## ExcessMTL
_parser.add_argument(
    "--robust_step_size", default=1e-2, type=float, help="step size"
)
## FairGrad
...
_parser.add_argument(
    "--min_delta", type=float, default=0.0, help="min_delta for early stopping"
)
...
_parser.add_argument(
    "--kgamma", type=float, default=1.0, help="gamma for DSelect-k"
)
...
def prepare_args(params):
    ...
    kwargs["early_stopping"] = params.early_stopping
    kwargs["patience"] = params.patience
    kwargs["min_delta"] = params.min_delta
    if params.weighting in [
        ...
        if params.weighting in ["DWA"]:
            ...
            if params.GradVac_beta is not None:
                ...
                kwargs["weight_args"]["GradVac_group_type"] = (
                    params.GradVac_group_type
                )
            else:
            ...
            if (
                ...
                kwargs["weight_args"]["update_weights_every"] = (
            ...
            kwargs["weight_args"]["STCH_warmup_epoch"] = (
                params.STCH_warmup_epoch
            )
        ...
        raise ValueError(
            "No support weighting method {}".format(params.weighting)
        )
    ...
    if params.arch in [
        ...
        raise ValueError(
            "No support architecture method {}".format(params.arch)
        )
    ...
    if params.scheduler is not None:
        if params.scheduler in ["step", "cos", "exp"]:
            ...
            raise ValueError(
                "No support scheduler method {}".format(params.scheduler)
            )
    else:
...
def _display(params, kwargs, optim_param, scheduler_param):
    ...
    for wa, p in zip(
        ["weight_args", "arch_args"], [params.weighting, params.arch]
    ):
        if kwargs[wa] != {}:
